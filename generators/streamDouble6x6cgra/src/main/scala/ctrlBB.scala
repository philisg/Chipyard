...
class CtrlBBModule(implicit val p: Parameters) extends Module
  ...
  val memory_data       = Reg(init = Vec.fill(6) { 0.U(32.W) })
  ...
  * input length: we tell the CGRA how long the input is, so that it knows
  ...
  io.from_mem0    := memory_data(0)
  io.from_mem1    := memory_data(1)
  io.from_mem2    := memory_data(2)
  io.from_mem3    := memory_data(3)
  io.from_mem4    := memory_data(4)
  io.from_mem5    := memory_data(5)
  ...
  switch(cgra_state){
    ...
    is(c_get_address){
      when(io.addr0 =/= last_address && address_counter =/= input_len(1)){
        ...
        cgra_clock_en   := false.B
        when(io.mem_req_rdy){
          cgra_state    := c_get_second_address
  ...
  val receive :: Nil    = Enum(UInt(), 1)
  val mem_rec_state     = Reg(init = receive)
    ...
    is(receive){
      when(io.mem_resp_val){
        ...
        when(io.mem_resp_tag === tag){
          ...
          io.from_mem1    := io.mem_resp_data(31,16)
          memory_data(1)  := io.mem_resp_data(31,16)
        }.elsewhen(io.mem_resp_tag === tag + 1){
          io.from_mem5    := io.mem_resp_data(31,0)
          memory_data(5)  := io.mem_resp_data(31,0)
        }
      }
      when(receive_counter === UInt(2)){
        receive_counter := 0
        cgra_clock_en   := true.B
        tag             := tag + 2
      }
  ...
  }.elsewhen(sampling_en && (output_counter === ((input_len(1)/6)+2))){
    when(has_output){
