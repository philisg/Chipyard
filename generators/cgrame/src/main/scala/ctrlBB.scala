...
class CtrlBBModule(implicit val p: Parameters) extends Module
  ...
  //memory handler
  val m_idle :: m_preRead :: m_read :: m_wait :: m_read_CGRA :: m_wait_CGRA :: m_write_CGRA :: Nil = Enum(UInt(), 7)
  ...
  * input length: we tell the CGRA how long the input is, so that it knows
  ...
  switch(mem_s){
    ...
    is(m_wait){
      when(io.mem_resp_val && io.mem_resp_tag === 4){
        busy          := Bool(false)
