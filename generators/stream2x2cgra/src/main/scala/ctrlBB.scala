...
class CtrlBBModule(implicit val p: Parameters) extends Module
  ...
  //memory handler
  val m_idle :: m_read_CGRA :: m_wait_CGRA :: m_write_CGRA :: m_write_output :: Nil = Enum(UInt(), 5)
  ...
  * input length: we tell the CGRA how long the input is, so that it knows
  ...
  when(io.from_cgra1 =/= output_data || sampling_en && clock_reg){
    output_counter := output_counter + 1
    output_data    := io.from_cgra1
    when(output_counter === input_len(1)){
      when(has_output){
