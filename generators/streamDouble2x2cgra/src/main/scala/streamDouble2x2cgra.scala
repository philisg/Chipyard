...
case object streamDouble2x2cgraWidthP extends Field[Int]
case object streamDouble2x2cgraStages extends Field[Int]
case object streamDouble2x2cgraFastMem extends Field[Boolean]
case object streamDouble2x2cgraBufferSram extends Field[Boolean]
...
class streamDouble2x2cgraAccel(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(
    opcodes = opcodes, nPTWPorts = 0) {
    override lazy val module = new streamDouble2x2cgraAccelImp(this)
}

class streamDouble2x2cgraAccelImp(outer: streamDouble2x2cgraAccel)(implicit p: Parameters) extends LazyRoCCModuleImp(outer){
  
  val streamDouble2x2cgrabb   = Module(new Cgra2x2BlackBox)
  ...
  //CGRA IO
  streamDouble2x2cgrabb.io.CGRA_Clock     := ctrl.io.CGRA_Clock
  streamDouble2x2cgrabb.io.Config_Clock   := ctrl.io.Config_Clock
  streamDouble2x2cgrabb.io.CGRA_Reset     := reset
  streamDouble2x2cgrabb.io.Config_Reset   := ctrl.io.Config_Reset
  streamDouble2x2cgrabb.io.ConfigIn       := ctrl.io.cgra_Inconfig
  ctrl.io.cgra_Outconfig            := streamDouble2x2cgrabb.io.ConfigOut
  ctrl.io.from_cgra0                <> streamDouble2x2cgrabb.io.dataOut0
  ctrl.io.from_cgra1                <> streamDouble2x2cgrabb.io.dataOut1
  streamDouble2x2cgrabb.io.dataIn0        := ctrl.io.to_cgra0
  streamDouble2x2cgrabb.io.dataIn1        := ctrl.io.to_cgra1
  streamDouble2x2cgrabb.io.write0         := ctrl.io.write0
  streamDouble2x2cgrabb.io.write1         := ctrl.io.write1

  ctrl.io.write_rq0                 := streamDouble2x2cgrabb.io.write_rq0
  ctrl.io.write_rq1                 := streamDouble2x2cgrabb.io.write_rq1
  ctrl.io.to_mem0                   := streamDouble2x2cgrabb.io.to_mem0
  ctrl.io.to_mem1                   := streamDouble2x2cgrabb.io.to_mem1
  ctrl.io.addr0                     := streamDouble2x2cgrabb.io.addr0
  ctrl.io.addr1                     := streamDouble2x2cgrabb.io.addr1
  streamDouble2x2cgrabb.io.from_mem0      := ctrl.io.from_mem0
  streamDouble2x2cgrabb.io.from_mem1      := ctrl.io.from_mem1
...
class WithstreamDouble2x2cgraAccel extends Config ((site, here, up) =>{
    case Cgra2x2BlackBox => true
    case BuildRoCC      => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      val streamDouble2x2cgra = LazyModule.apply(new streamDouble2x2cgraAccel(OpcodeSet.custom0)(p))
      streamDouble2x2cgra
    }
  )
})

class WithstreamDouble2x2cgraBlackBox extends Config((site, here, up) => {
  case Cgra2x2BlackBox => true
  // case streamDouble2x2cgraTLB => None // Do not use the more correct DmemModule when blackboxing
}) 
