...
case object stream6x6cgraWidthP extends Field[Int]
case object stream6x6cgraStages extends Field[Int]
case object stream6x6cgraFastMem extends Field[Boolean]
case object stream6x6cgraBufferSram extends Field[Boolean]
...
class stream6x6cgraAccel(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(
    opcodes = opcodes, nPTWPorts = 0) {
    override lazy val module = new stream6x6cgraAccelImp(this)
}

class stream6x6cgraAccelImp(outer: stream6x6cgraAccel)(implicit p: Parameters) extends LazyRoCCModuleImp(outer){
...
class Withstream6x6cgraAccel extends Config ((site, here, up) =>{
    case Cgra6x6BlackBox => true
    case BuildRoCC      => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      val stream6x6cgra = LazyModule.apply(new stream6x6cgraAccel(OpcodeSet.custom0)(p))
      stream6x6cgra
    }
  )
})
...
class WithCgra6x6BlackBox extends Config((site, here, up) => {
  case Cgra6x6BlackBox => true
  // case stream6x6cgraTLB => None // Do not use the more correct DmemModule when blackboxing
}) 
